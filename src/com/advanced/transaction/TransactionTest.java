package com.advanced.transaction;
 ...
 * 		> 一组逻辑操作单元：一个或多个DML（数据库操纵语言，增删改）操作。
 * 
...
public class TransactionTest {
	...
	public void testUpdateWithTx() {
		...
		try {
			...
			e.printStackTrace();
			//3.回滚数据，出现异常就直接到 catch 中了，就需要对执行的操作进行回滚
			try {
				conn.rollback();
	...
	//通用的查询操作，用于返回数据表中的一条记录（version 2.0：考虑上事务），传进一个连接
	public <T> T getInstance(Connection conn,Class<T> clazz,String sql, Object... args) {
		...
		try {
			...
			JDBCUtils.closeResource(null, ps, rs);//不关闭连接
